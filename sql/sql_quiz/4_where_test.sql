...
select cmf, PARTY_NM, coalesce(PIF_AMT, 0) + coalesce(INST_AMT, 0)
	+ coalesce(OVRS_AMT, 0) + coalesce(CASH_AMT, 0) as '총사용금액'
    from card_tran_201311
    order by 총사용금액 desc;

-- 2. 신규 무이자 할부 카드의 홍보대상 고객을 추출하시오
select PARTY_NM from card_tran_201311
	where INST_AMT is null;

-- 3. pb고객을 대상으로 일시불사용금액의 1%를 캐시백해주는 캠페인을 진행하면 누가 얼마의 금액을 받을 것인가
select PARTY_NM,coalesce(PIF_AMT, 0)*0.01 as '캐시백'
	from card_tran_201311
    where seg='pb';

-- 4. 고객 등급별 총사용 금액을 추출하고 총사용금액 기준 내림차순 정렬
select seg, coalesce(PIF_AMT, 0) + coalesce(INST_AMT, 0)
	+ coalesce(OVRS_AMT, 0) + coalesce(CASH_AMT, 0) as '총사용금액'
    from card_tran_201311
    group by seg
    order by 총사용금액 desc;
...
-- 6. 사용횟수가 가장 많은 서비스는
select count(PIF_AMT), count(INST_AMT), count(OVRS_AMT), count(CASH_AMT)
	from card_tran_201311;
...
select purchase_amt/purchase_cnt as '평균구매금액'
	from purchase_tran;


-- 5. 올해와 작년의 구매건수 당 평균 구매금액을 구하시오
select purchase_amt/purchase_cnt as '올해 평균구매금액',
	last_amt/last_cnt as '작년 평균구매금액'
	from purchase_tran;
